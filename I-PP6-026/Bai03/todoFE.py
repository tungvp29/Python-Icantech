...
import uuid
from todoBE import save_data, add_todo, update_todo, delete_todo, toggle_complete, filter_todos, load_todos, load_groups
...
def display_todo_form(todo=None):
    ...
    with st.form(key=f"todo_form_{todo['id'] if is_edit else 'new'}"):
        ...
        # Upload hình ảnh
        uploaded_file = st.file_uploader("Hoặc upload hình ảnh", type=['png', 'jpg', 'jpeg', 'gif', 'pdf'])
        if uploaded_file:
            # Lưu file upload
            upload_dir = "uploads"
