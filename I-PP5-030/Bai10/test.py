...
while status:
  ...
  if camera_available:
    ...
    if ret:
      ...
      # Kiểm tra cử chỉ open hand để khởi động game (dùng frame gốc để độ chính xác cao hơn)
      ...
      # Kiểm tra cử chỉ nắm tay để nhảy (khi game đang chạy)
      ...
      # Hiển thị hướng dẫn trên camera (trên frame nhỏ)
      if waiting_for_gesture:
        cv2.putText(frame_small, "OPEN HAND to start", (10, new_height-40), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 1)
      elif game_started and not pausing:
        cv2.putText(frame_small, "FIST to JUMP (or press F)", (10, new_height-40), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 255), 1)
      
      # Thêm thông tin điều khiển
      cv2.putText(frame_small, "Press 'q' to quit", (10, new_height-20), 
                  cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200, 200, 200), 1)
      
      # Hiển thị camera feed nhỏ
      cv2.imshow('Hand Gesture Control', frame_small)
  else:
  ...
  for event in pygame.event.get():
    ...
    if event.type == pygame.KEYDOWN:
      if event.key == pygame.K_SPACE:
        ...
        if robot_y == 230:
          jump = True
          print("Robot nhảy bằng phím F (test fist gesture)!")
